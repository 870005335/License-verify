...
public class DateUtils {
    ...
    public static Long getTime(String time) throws LicenseCommonException {
        if (StringUtils.isEmpty(time)) {
            throw new LicenseCommonException("时间[" + time + "]格式不合法");
    ...
    public static synchronized Date str2Date(String time) throws LicenseCommonException {
        sDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

        try {
            return sDateFormat.parse(time);
        } catch (Exception var2) {
            throw new LicenseCommonException("字符串[" + time + "]转换日期格式异常");
        }
    }
